...
export default function ModalPets({ data }) {
  const [isVisible, setIsVisible] = useState(false)

  const {editPetData} = usePets()
  const {active, setActive} = useUserStore()
  ...
  const handleModalSubmit = (e) => {
    ...
    editPetData(newPetData, data.id)
    .then(()=> closeModal())
  }
            ...
            createPortal(
                <div
                    ...
                    <div
                        ...
                        className="relative flex w-[98%] md:w-3/4 min-h-[400px] md:h-[80%] flex-col items-center justify-between rounded-md  bg-primary font-bold shadow-box shadow-[1px_1px_12px_#bbb] transition-all duration-300 overflow-hidden"
                        >
                            <button onClick={closeModal} className='flex justify-end w-full bg-bgBtn p-3'>
                                <p className="bg-bgHeader w-10 h-8 md:h-6 md:w-6 rounded-sm flex justify-center items-center">X</p>
                            </button>
                            <article className="flex items-center justify-between flex-col w-full h-full p-4">
                                <img
                                    ...
                                    src={data.images[0]?.url} alt="pet image" />
                                <section className="w-full flex items-center justify-start gap-3 flex-col">
                                    <form
                                        ...
                                        <div className='w-full'>
                                            <label className='font-bold' htmlFor="nickName">Nombre</label>
                                            <input 
                                                type="text" 
                                                id="nickName" 
                                                name='nickName' 
                                        ...
                                        <div className='w-full'>
                                            <label className='font-bold' htmlFor="birth">F. Nacimiento</label>
                                            <input 
                                                type="date" 
                                                id="birth" 
                                                name='birth' 
                                        ...
                                        <section className='flex justify-center items-center p-1 w-full'>
                                            <button
